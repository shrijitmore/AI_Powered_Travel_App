...
export default function App() {
  ...
  const planRoute = async () => {
    if (!location) {
      Alert.alert('Location Required', 'Please enable location services to plan routes.');
      return;
    }
    ...
    try {
      ...
      const sampleRoute = {
        ...
        start: {
          ...
          latitude: (location?.coords.latitude || 0) + 0.1,
          longitude: (location?.coords.longitude || 0) + 0.1,
          name: 'End Location',
        },
        route_type: 'scenic',
        waypoints: [],
      ...
      if (routeResponse.ok) {
        ...
        const completeResponse = await fetch(
          `${API_BASE}/api/routes/${createdRoute.id}/complete?user_id=${currentUser.id}`,
          {
            method: 'PATCH',
          }
        );
    ...
    <ScrollView style={styles.content}>
      <View style={styles.header}>
        ...
        <View style={styles.pointsContainer}>
          ...
          <Text style={styles.pointsText}>{currentUser?.total_points || 0} Points</Text>
        </View>
      </View>
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statNumber}>{currentUser?.routes_completed || 0}</Text>
          <Text style={styles.statLabel}>Routes Completed</Text>
        </View>
        <View style={styles.statCard}>
          <Ionicons name="trophy" size={24} color="#FF6B6B" />
          <Text style={styles.statNumber}>Level {currentUser?.level || 1}</Text>
          <Text style={styles.statLabel}>Explorer Level</Text>
        </View>
        <View style={styles.statCard}>
          <Ionicons name="ribbon" size={24} color="#4ECDC4" />
          <Text style={styles.statNumber}>{currentUser?.badges?.length || 0}</Text>
      ...
      <View style={styles.featuresSection}>
        ...
        <View style={styles.featureItem}>
          ...
          <Text style={styles.featureText}>Gamified challenges and point rewards</Text>
        </View>
        
        <View style={styles.featureItem}>
          <Ionicons name="people" size={20} color="#4ECDC4" />
          <Text style={styles.featureText}>Social leaderboards and achievements</Text>
        </View>
        
        <View style={styles.featureItem}>
          <Ionicons name="chatbubble" size={20} color="#FFD700" />
          <Text style={styles.featureText}>AI travel assistant for recommendations</Text>
        </View>
      </View>
    </ScrollView>
  );
    ...
    <ScrollView style={styles.content}>
      ...
      <View style={styles.profileStats}>
        <View style={styles.profileStatItem}>
          <Text style={styles.profileStatNumber}>{currentUser?.total_points || 0}</Text>
          <Text style={styles.profileStatLabel}>Total Points</Text>
        </View>
        <View style={styles.profileStatItem}>
          <Text style={styles.profileStatNumber}>{currentUser?.routes_completed || 0}</Text>
          <Text style={styles.profileStatLabel}>Routes</Text>
        </View>
        <View style={styles.profileStatItem}>
          <Text style={styles.profileStatNumber}>Level {currentUser?.level || 1}</Text>
